...
class LanguageMenu(QWidget):
    def __init__(self, start_callback):
        ...
        layout.addWidget(QLabel("Text Language (OCR):"))
    ...
    def find_tessdata_path(self):
        ...
        if not tesseract_cmd: return None
    ...
    def tess_to_iso(self, tess_code):
        special = {"kor": "ko"}
        if tess_code in special: return special[tess_code]
        try:
            lang = pycountry.languages.get(alpha_3=tess_code)
            if lang and hasattr(lang, "alpha_2"): return lang.alpha_2
        except: pass
        return tess_code[:2]

    def load_ocr_languages(self):
        path = self.find_tessdata_path()
        if not path:
    ...
    def load_translation_targets(self):
        try:
            ...
            for lang in response.json():
                self.trans_combo.addItem(f"{lang['name']} ({lang['code']})", lang['code'])
        except:
    ...
    def start_clicked(self):
        ...
        if ocr_code and target_code:
            source_code = self.tess_to_iso(ocr_code)
            self.hide()
